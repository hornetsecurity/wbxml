...
testExample1 = testCase "Example 1" $
    case AP.parseOnly (wbxmlDocument <* AP.endOfInput) input of
        ...
        Right Document{..} -> do
            ...
            elementName documentRoot
                @?= TaggedElementName (CodepageReference 0 7)
            ...
            elementContents e
                @?= [ StrContent (Inline " X & Y")
                    , ElementContent (Element (TaggedElementName (CodepageReference 0
                                                                                    5))
                                              []
                                              [])
                    , StrContent (Inline " X")
...
testExample2 = testCase "Example 2" $
    case AP.parseOnly (wbxmlDocument <* AP.endOfInput) input of
        Left e -> assertFailure e
        Right Document{..} -> do
            let Header{..} = documentHeader
            ...
            elementAttributes e
                @?= [ Attribute (TaggedAttributeStart $ CodepageReference 0 9)
                                [ StrAttributeValue (Reference 0) ]
                    , Attribute (TaggedAttributeStart $ CodepageReference 0 5)
                                []
                    ]

            let [ ElementContent e1, StrContent s, ElementContent e2 ] =
                    elementContents e

            elementName e1 @?= TaggedElementName (CodepageReference 0 8)
            elementAttributes e1
                @?= [ Attribute (TaggedAttributeStart $ CodepageReference 0 6)
            ...
            elementAttributes e2
                @?= [ Attribute (TaggedAttributeStart $ CodepageReference 0 7)
                                []
...
testParser :: TestTree
testParser = testGroup "Data.Wbxml.Parser" [ testExample1, testExample2 ]
