...
testSegmentString = testCase "segmentString" $ do
    segmentString trie "" @?= []

    segmentString trie "foo" @?= [ ("foo", Just 0) ]
    segmentString trie "quux" @?= [ ("quux", Nothing) ]

    segmentString trie "foobarbaz"
        @?= [ ("foo", Just 0), ("bar", Just 1), ("baz", Just 2) ]

    segmentString trie "the foo and the bar"
        @?= [ ("the ", Nothing)
            , ("foo", Just 0)
            , (" and the ", Nothing)
            , ("bar", Just 1)
  ...
  where
    trie = Trie.fromList [ ("foo", 0 :: Int), ("bar", 1), ("baz", 2) ]
...
testToXmlName = testCase "Name" $ do
...
testToXmlAttribute = testCase "Attribute" $ do
    decode (toXmlAttribute $ Attribute (TaggedAttributeStart (cp 0x4a)) [])
        @?= Right (attr "href" [ "" ])
    decode (toXmlAttribute $ Attribute (LiteralAttributeStart 0) [])
        @?= Right (attr "foo" [ "" ])

    decode (toXmlAttribute $
                Attribute (TaggedAttributeStart (cp 0x4c))
...
testToXmlInstruction = testCase "Instruction" $ do
    decode (toXmlInstruction $
                ProcessingInstruction (TaggedAttributeStart (cp 0x21)) [])
        @?= Right (XML.Instruction "name" "")

    decode (toXmlInstruction $
                ProcessingInstruction (LiteralAttributeStart 0)
                                      [ StrAttributeValue (Inline "lorem") ])
        @?= Right (XML.Instruction "foo" "lorem")

testToXmlNode :: TestTree
testToXmlNode = testCase "Node" $ do
    decode (toXmlNode $
                ElementContent $ Element (TaggedElementName (cp 0x1c)) [] [])
...
testFromXmlAttribute = testCase "Attribute" $ do
    encode (fromXmlAttribute $ attr "href" [])
        @?= Right (Attribute (TaggedAttributeStart (cp 0x4a)) [])
    encode (fromXmlAttribute $ attr "foo" [])
        @?= Right (Attribute (LiteralAttributeStart 0) [])
...
testFromXmlInstruction =
    testCase "Instruction" $ do
        ...
        encode (fromXmlInstruction $ XML.Instruction "foo" "lorem")
            @?= Right (ProcessingInstruction (LiteralAttributeStart 0)
                                             [ StrAttributeValue (Inline "lorem")
                                             ])

testFromXmlNode :: TestTree
testFromXmlNode = testCase "Node" $ do
    encode (fromXmlNode $ XML.NodeElement $ XML.Element (name "a") [] [])
        @?= Right (Just (ElementContent $
                             Element (TaggedElementName (cp 0x1c)) [] []))
    ...
    encode (fromXmlNode $ XML.NodeInstruction $ XML.Instruction "foo" "")
        @?= Right (Just (ProcessingInstructionContent $
                             ProcessingInstruction (LiteralAttributeStart 0) []))
