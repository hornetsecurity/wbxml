...
codepageSwitch cp = switch >>= modify . cp
    where switch = token tokenSwitchPage *> word8
...
str :: Parser Str
str = AP.choice [ token tokenStrI *> termStr
                , token tokenStrT *> indexStr
                ]
...
extension = do
  ...
  if (tok .&. 0xc0) == 0xc0 then return $ Extension eid Nothing
  else if (tok .&. 0xc0) == 0x80 then Extension eid . Just <$> indexStr
...
element = AP.option () (codepageSwitch (\p s -> s { stateTagPage = p })) *> do
  tok <- validToken
  let code = tok .&. 0x3f
  name <- if code == tokenLiteral then LiteralElementName <$> word32 else TaggedElementName <$> codepageReference stateTagPage (pure code)
...
processingInstruction = token tokenPi *> (ProcessingInstruction <$> attributeStart <*> AP.manyTill attributeValue (token tokenEnd))
