...
mkDecoderData :: DocumentType
              -> (ByteString -> Except String Text)
              -> StrTable
              -> DecoderData
...
decodeAttrSeq start values = do
  ...
  where
    ...
    decodeAttrValue v = case v of
        ...
        ExtensionAttributeValue _ ->
            lift $ throwE "Decoding WBXML Extension Tokens not supported"
...
decodeDocument :: DocumentType
               -> (ByteString -> Except String Text)
               -> Document
               -> Either String XML.Document
decodeDocument dt decText doc =
    runDecoder (toXmlDocument doc)
               (mkDecoderData dt decText (documentStrTable doc))
...
encodeAttributeStart :: ByteString
                     -> ByteString
                     -> Encoder (AttributeStart, ByteString)
encodeAttributeStart n v = do
    ...
    case Trie.match trie str of
        Just (m, cr, rest) | BS.length m > BS.length n ->
                                 return (TaggedAttributeStart cr, rest)
        _ -> do
...
encodeAttributeValue str = do
  ...
  where
    translate (bs, Nothing) = StrAttributeValue <$> encodeString bs
...
fromXmlAttribute (name, values) = do
  ...
  where
    ...
    unpack (XML.ContentEntity _) =
        lift $ throwE "Encoding ContentEntity not supported"
...
fromXmlNode (XML.NodeContent (XML.ContentText t)) =
    Just . StrContent <$> fromXmlString t
fromXmlNode (XML.NodeContent (XML.ContentEntity _)) =
    lift $ throwE "Encoding ContentEntity not supported"
fromXmlNode (XML.NodeInstruction i) =
    Just . ProcessingInstructionContent <$> fromXmlInstruction i
...
encodeDocument :: DocumentType
               -> (Text -> Except String ByteString)
               -> Header
               -> XML.Document
               -> Either String Document
encodeDocument dt encText hdr doc =
    evalEncoder (fromXmlDocument hdr doc) (mkEncoderData dt encText)
