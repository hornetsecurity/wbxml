...
codepageReference :: (PrinterState -> Word8)
                  -> (Word8 -> PrinterState -> PrinterState)
                  -> CodepageReference
                  -> Printer ()
codepageReference g s (CodepageReference cp ref) = do
    cur <- lift $ gets g
    let switch = if cp /= cur
                 then lift (modify (s cp)) >> codepageSwitch cp
                 else return mempty
    switch >> word8 ref

attrCodepageReference :: CodepageReference -> Printer ()
attrCodepageReference = codepageReference stateAttrPage
                                          (\p s -> s { stateAttrPage = p })

tagCodepageReference :: CodepageReference -> Printer ()
tagCodepageReference = codepageReference stateTagPage
                                         (\p s -> s { stateTagPage = p })
...
version Version{..} = word8 tok
  where
    tok = (((versionMajor - 1) `shiftL` 4) .&. 0xf0) .|. (versionMinor .&. 0x0f)
...
extension (Extension eid (Just (Reference i))) =
    word8 (0x80 .|. eid) >> word32 i
extension (Extension eid (Just (Inline bs))) =
    word8 (0x40 .|. eid) >> termStr bs
...
attribute (Attribute start values) =
    attributeStart start >> mapM_ attributeValue values
...
element (Element name attributes contents) = n >> attrs >> conts
  where
    ...
    n = case name of
        TaggedElementName (CodepageReference cp i) ->
            tagCodepageReference (CodepageReference cp (i .|. mask))
